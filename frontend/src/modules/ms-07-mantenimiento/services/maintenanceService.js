...
class MaintenanceService {
    ...
    async request(endpoint, options = {}) {
        ...
        if (token) {
            ...
            console.warn('⚠️ No hay token de autenticación. Las peticiones pueden fallar.');
        }
        ...
        try {
            ...
            if (cleanData.attachedDocuments) {
                ...
                if (cleanData.attachedDocuments.length !== 1) {
                    throw new Error('Debe proporcionar exactamente 1 documento con fileUrl');
                }
                if (!cleanData.attachedDocuments[0].fileUrl) {
