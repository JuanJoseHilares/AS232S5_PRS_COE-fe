...
import { getCategorias, getAreas, getProveedores, getResponsables, getUbicaciones } from '../../services/apis-dev';
...
export default function AssetModal({ isOpen, onClose, onSuccess, bien = null }) {
  ...
  // Cargar listas de dev-support (proveedores, ubicaciones, responsables, areas)
  useEffect(() => {
    let mounted = true;
    const load = async () => {
      try {
        ...
        if (mounted) {
          const normProviders = (p || []).map(x => ({ id: x.id || x._id || x.codigoProveedor || x.codigo || '', label: x.razonSocial || x.nombre || x.codigoProveedor || '', code: x.codigoProveedor || x.codigo || '', raw: x }));
          const normUbicaciones = (u || []).map(x => ({ id: x.id || x._id || x.codigoUbicacion || x.codigo || '', label: x.nombre || x.codigoUbicacion || '', code: x.codigoUbicacion || x.codigo || '', raw: x }));
          const normResponsables = (r || []).map(x => ({ id: x.id || x._id || x.personaId || x.codigoEmpleado || '', label: x.nombres || x.razonSocial || x.codigoEmpleado || '', code: x.codigoEmpleado || x.personaId || '', raw: x }));
          const normAreas = (a || []).map(x => ({ id: x.id || x._id || x.codigoArea || x.codigo || '', label: x.nombre || x.codigoArea || '', code: x.codigoArea || x.codigo || '', raw: x }));

          setProviders(normProviders);
          setUbicaciones(normUbicaciones);
          setResponsables(normResponsables);
          setAreas(normAreas);
        }
        console.log('Datos de dev-support cargados:', { p, u, r, a });
      } catch (err) {
        console.error('Error cargando datos dev-support', err);
        if (mounted) setDevError('No se pudieron cargar listas de soporte');
      }
    };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (mounted) setCategoriesError('No se pudieron cargar las categorías');
      }
    };
